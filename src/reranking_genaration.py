...
class QueryRouter:
    def __init__(self):
        ...
        try:
            self.classifier.load_pipeline(settings.classifier_model)
        ...
        try:
            ...
            base_retriever = vs.as_retriever(search_kwargs={"k": self.top_k})
            self.global_retriever = (
                ContextualCompressionRetriever(
                    base_compressor=self.compressor,
                    base_retriever=base_retriever
                )
                if self.compressor else base_retriever
            )
            logger.info("✅ Global retriever initialized with reranking.")
        except Exception as e:
        ...
        for category in all_categories:
            try:
                ...
                base_retriever = vs.as_retriever(
                    search_kwargs={"k": self.top_k, "filter": {"category": category}}
                )
                self.retrievers[category] = (
                    ContextualCompressionRetriever(
                        base_compressor=self.compressor,
                        base_retriever=base_retriever
                    )
                ...
                logger.info(f"✅ Retriever cached for category: {category} (with reranking).")
            except Exception as e:
